...
public struct Deque<Element> {
    ...
    fileprivate(set) var buffer: DequeBuffer<Element>
...
extension Deque {
    ...
    var isUnique: Bool { mutating get { isKnownUniquelyReferenced(&buffer) } }

    @inline(__always)
    private mutating func makeUnique() {
        makeUnique(buffer.capacity)
    }
...
extension Deque: ExpressibleByArrayLiteral {
    public init(arrayLiteral elements: Element...) {
        buffer = DequeBuffer(capacity: elements.count)
...
extension Deque: CustomStringConvertible, CustomDebugStringConvertible {
    ...
    public var description: String {
        makeDescription(debug: false)
    }

    public var debugDescription: String {
        makeDescription(debug: true)
    }
}
...
extension Deque: RangeReplaceableCollection {
    ...
    public mutating func replaceSubrange(_ range: Range<Int>, with newElements: some Collection<Element>) {
        ...
        if isUnique, count + delta <= capacity {
            ...
            b.insert(contentsOf: buffer, subrange: 0 ..< range.lowerBound, at: 0)
            b.insert(contentsOf: newElements, at: b.count)
            b.insert(contentsOf: buffer, subrange: range.upperBound ..< count, at: b.count)
            buffer = b
        }
    }
    ...
    // Code targeting the Swift 4.1 compiler and below
        ...
        public mutating func replaceSubrange(_ range: CountableRange<Int>, with newElements: some Collection<Element>) {
            ...
            replaceSubrange(Range(range), with: newElements)
        }
    #endif

    public mutating func replaceSubrange(_ range: ClosedRange<Int>, with newElements: some Collection<Element>) {
        // This is also defined as a protocol extension on RangeReplaceableCollection. However, using that extension
        // breaks isUnique, leading to extra COW copies. Providing this overload restores COW behavior in static contexts, at least.
        replaceSubrange(Range(range), with: newElements)
    }
    ...
    // Code targeting the Swift 4.1 compiler and below
    #if !(swift(>=4.1.50) || (swift(>=3.4) && !swift(>=4.0)))
        public mutating func replaceSubrange(_ range: CountableClosedRange<Int>, with newElements: some Collection<Element>) {
            // This is also defined as a protocol extension on RangeReplaceableCollection. However, using that extension
            // breaks isUnique, leading to extra COW copies. Providing this overload restores COW behavior in static contexts, at least.
            replaceSubrange(Range(range), with: newElements)
        }
    ...
    ///
    /// - Complexity: O(`count`). Note though that complexity is O(1) if `i` is of a constant distance from the front or end of the deque.
    public mutating func insert(contentsOf newElements: some Collection<Element>, at i: Int) {
...
final class DequeBuffer<Element> {
    ...
    init(capacity: Int = 16) {
        ...
        elements = UnsafeMutablePointer.allocate(capacity: capacity)
        self.capacity = capacity
        count = 0
        start = 0
    }

    convenience init(repeating: Element, count: Int) {
    ...
    func realloc(_ capacity: Int) -> DequeBuffer {
        ...
        if start + count <= self.capacity {
            ...
            let c = self.capacity - start
            dst.moveInitialize(from: src.advanced(by: start), count: c)
            dst.advanced(by: c).moveInitialize(from: src, count: count - c)
        }
        count = 0
        return buffer
    }
    ...
    func bufferIndex(forDequeIndex index: Int) -> Int {
        let i = start + index
    ...
    func dequeIndex(forBufferIndex i: Int) -> Int {
        if i >= start {
            return i - start
        }
    ...
    func popFirst() -> Element? {
        ...
        start = bufferIndex(forDequeIndex: 1)
        count -= 1
        return first
    }
    ...
    func insert(contentsOf buffer: DequeBuffer, at index: Int) {
        insert(contentsOf: buffer, subrange: 0 ..< buffer.count, at: index)
    }

    func insert(contentsOf buffer: DequeBuffer, subrange: Range<Int>, at index: Int) {
        assert(buffer !== self)
        assert(index >= 0 && index <= count)
        ...
        let dstStart = bufferIndex(forDequeIndex: index)
        let srcStart = buffer.bufferIndex(forDequeIndex: subrange.lowerBound)

        let srcCount = subrange.count

        let dstEnd = bufferIndex(forDequeIndex: index + srcCount)
    ...
    func insert(contentsOf collection: some Collection<Element>, at index: Int) {
        assert(index >= 0 && index <= count)
    ...
    func replaceSubrange(_ range: Range<Int>, with newElements: some Collection<Element>) {
        let newCount: Int = numericCast(newElements.count)
        let delta = newCount - range.count
...
public extension Deque {
    ...
    func map<T>(_ transform: (Element) throws -> T) rethrows -> [T] {
        ...
        try forEach { try result.append(transform($0)) }
        return result
    }
