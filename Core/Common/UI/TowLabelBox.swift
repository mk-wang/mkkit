...
open class TowLabelBox: MKBaseView {
    ...
    public let layoutBloack: ((TowLabelBox, UILabel, UILabel) -> Void)?

    public init(frame: CGRect, config: Config, layoutBloack: ((TowLabelBox, UILabel, UILabel) -> Void)? = nil) {
        self.config = config
        self.layoutBloack = layoutBloack
    ...
    override open func readyToLayout() {
        if let layoutBloack {
            layoutBloack(self, topLbl, bottomLbl)
        } else {
